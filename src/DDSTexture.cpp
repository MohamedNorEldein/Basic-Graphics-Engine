...
typedef struct
{
    ...
    UINT      miscFlag; // See D3D11_RESOURCE_MISC_FLAG
    UINT      arraySize;
    UINT      reserved;
} DDS_HEADER_DXT10;
    ...
    )
{
    if (!bitData || !initData)
    {
        printf("Platform::InvalidArgumentException\n");
        return;
    }
    ...
    for (size_t j = 0; j < arraySize; j++)
    {
        ...
        for (size_t i = 0; i < mipCount; i++)
        {
            ...
            {
                printf("Platform::OutOfBoundsException\n");
                return;
            }
    ...
    if (!index)
    {
        printf("Platform::FailureException()\n");
        return;
    ...
    )
{
    ...
    if (ddsDataSize < (sizeof(UINT) + sizeof(DDS_HEADER)))
    {
        printf("Platform::FailureException\n");
        return;
    }
        ...
        (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC))
    {
        ...
        if (ddsDataSize < (sizeof(DDS_HEADER) + sizeof(UINT) + sizeof(DDS_HEADER_DXT10)))
        {
            printf("Platform::FailureException\n");
            return;
        }
    ...
    ptrdiff_t offset = sizeof(UINT) + sizeof(DDS_HEADER) + (bDXT10Header ? sizeof(DDS_HEADER_DXT10) : 0);
